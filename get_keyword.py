...
def q2(domain):
    #rs = domain.select("select name,oneDayChange,postDate from t_enriched_bloomberg_prices",max_items=3)
    rs = domain.select("select country,date,stockIndex,holidayName from s_holiday",max_items=3)
    for r in rs:
        print r

def delete_domain(key_id,secret,domain):
